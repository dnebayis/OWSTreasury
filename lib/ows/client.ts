...
// Resolve ows binary — try multiple locations for Vercel compatibility
const OWS_BIN = (() => {
  const candidates = [
    path.join(process.cwd(), "node_modules", ".bin", "ows"),
    path.join(process.cwd(), "node_modules", "@open-wallet-standard", "core", "bin", "ows.js"),
    path.join("/var/task", "node_modules", ".bin", "ows"),
    path.join("/var/task", "node_modules", "@open-wallet-standard", "core", "bin", "ows.js"),
  ];
  for (const candidate of candidates) {
    try {
      require("fs").accessSync(candidate);
      // If it's a .js file, run with node explicitly
      return candidate.endsWith(".js") ? `node "${candidate}"` : `"${candidate}"`;
    } catch {
      // not found, try next
    }
  }
  return "ows"; // rely on PATH as last resort
})();
